...
6. For existing customizations, follow \`${frameworkPath}/setup/MIGRATION.INSTRUCTIONS.MD\`.
...
function classifyArtifact(rel) {
  ...
  if (/\.prompt\.md$/i.test(base) || norm.startsWith('.github/prompts/')) {
    return { type: 'Prompt', canonical: 'prompt-engineering/', action: 'REVIEW/COPY' };
  }
...
async function writeMigrationPlan(projectRoot, outputRelative) {
  ...
  const lines = [
    '# MIGRATION PLAN',
    '',
    '> Generated by the local Portable Agent Control Framework MCP server. This is a dry-run inventory. Review `setup/MIGRATION.INSTRUCTIONS.MD` before applying changes.',
    '',
